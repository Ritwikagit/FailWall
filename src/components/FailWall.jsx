...
const FailWall = () => {
  ...
  const handleReact = (postId, emoji) => {
    const existingKey = Object.keys(userReactions).find((key) => key.startsWith(`${postId}-`));
        ...
        <div className="space-y-4">
          {posts.map((post) => (
            ...
            >
              ...
              <div className="mt-3 flex flex-wrap gap-2 text-sm">
                {predefinedReactions.map((r) => {
                  const reactedKey = Object.keys(userReactions).find((key) => key.startsWith(`${post.id}-`));
                  const isDisabled = reactedKey && !reactedKey.endsWith(r.emoji);
                  return (
                    <button
                      key={r.emoji}
                      onClick={() => handleReact(post.id, r.emoji)}
